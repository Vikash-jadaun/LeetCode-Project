...
const getInitialCodeForLanguage = (problemData, selectedLanguage) => {
  const languageKey = langMap[selectedLanguage] || selectedLanguage;
  const entry = problemData?.startCode?.find((sc) => sc.language?.toLowerCase() === languageKey?.toLowerCase());
...
//   };
  ...
  useEffect(() => {
    const fetchProblem = async () => {
      ...
      try {
        ...
        setCode(getInitialCodeForLanguage(problemData, selectedLanguage));
        setLoading(false);
      } catch (error) {
  ...
  useEffect(() => {
    if (problem) {
      setCode(getInitialCodeForLanguage(problem, selectedLanguage));
    }
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      setRunResult({
        success: false,
        error: error.response?.data?.message || error.response?.data || 'Internal server error',
        testCases: []
      });
      setLoading(false);
      setActiveRightTab('testcase');
                    ...
                    {runResult.success ? (
                      ...
                      <div>
                        ...
                        <div className="mt-4 space-y-2">
                          ...
                          {runResult.error && (
                            <div className="bg-base-100 p-3 rounded text-xs">
                              {runResult.error}
                            </div>
                          )}
                        </div>
